...
let rec string_of_expr (e : expr) : string =
  ...
  | App (e1, e2) -> unparse_two "App" e1 e2
  | If (e1, e2, e3) ->
      "If(" ^ string_of_expr e1 ^ ", " ^ string_of_expr e2 ^ ", "
      ^ string_of_expr e3 ^ ")"
  | Let (s, r, e1, e2) ->
      "Let(" ^ s ^ ", " ^ string_of_bool r ^ "," ^ string_of_expr e1 ^ ", "
      ^ string_of_expr e2 ^ ")"
  | Record rs ->
      "Record("
      ^ string_of_list
          (fun (Lab rl, re) -> "\"" ^ rl ^ "\": " ^ string_of_expr re)
          rs
      ^ ")"
  | Select (Lab l, e) -> "Select(\"" ^ l ^ "\", " ^ string_of_expr e ^ ")"
